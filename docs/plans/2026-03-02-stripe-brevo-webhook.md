# Stripe → Brevo Webhook Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Automatically sync Stripe payment data (donors + members) to Brevo contacts via a PHP webhook.

**Architecture:** Single monolithic PHP script (`public/api/stripe-webhook.php`) following the existing `create-checkout.php` pattern. No Composer — raw cURL calls to Stripe and Brevo APIs. Retry + fallback file + email notification for Brevo failures.

**Tech Stack:** PHP (OVH mutualisé), Stripe webhook API, Brevo REST API v3, GitHub Actions (sed injection)

**Design doc:** `docs/plans/2026-03-02-stripe-brevo-webhook-design.md`

---

### Task 1: Update config.php with new variables

**Files:**
- Modify: `public/api/config.php`

**Step 1: Add the new config variables**

Open `public/api/config.php` and append the following after the existing variables:
...
```bash
git add public/api/config.php
git commit -m "feat: add webhook and Brevo config variables"
```

---

### Task 2: Create stripe-webhook.php — signature verification & event parsing

**Files:**
- Create: `public/api/stripe-webhook.php`

**Step 1: Create the file with signature verification and event routing**

Create `public/api/stripe-webhook.php` with the following content:
...
```bash
git add public/api/stripe-webhook.php
git commit -m "feat: add Stripe webhook for Brevo contact sync"
```

---

### Task 3: Update deploy.yml for production secrets

**Files:**
- Modify: `.github/workflows/deploy.yml`

**Step 1: Add sed commands after the existing Stripe key injection**

In `.github/workflows/deploy.yml`, after the existing "Inject Stripe secret key into config.php" step (line 22-25), add the new sed commands to that same step:

```yaml
      - name: Inject Stripe secret key into config.php
        run: |
          sed -i "s/sk_test_REPLACE_ME/${{ secrets.STRIPE_SECRET_KEY_LIVE }}/" dist/api/config.php
          sed -i "s/\$stripe_mode = 'test'/\$stripe_mode = 'live'/" dist/api/config.php
          sed -i "s/whsec_REPLACE_ME/${{ secrets.STRIPE_WEBHOOK_SECRET_LIVE }}/" dist/api/config.php
          sed -i "s/xkeysib-REPLACE_ME/${{ secrets.BREVO_API_KEY }}/" dist/api/config.php
          sed -i "s/\$brevo_list_adherents = 0/\$brevo_list_adherents = ${{ secrets.BREVO_LIST_ADHERENTS }}/" dist/api/config.php
          sed -i "s/\$brevo_list_donateurs = 0/\$brevo_list_donateurs = ${{ secrets.BREVO_LIST_DONATEURS }}/" dist/api/config.php
          sed -i "s/\$brevo_list_tous = 0/\$brevo_list_tous = ${{ secrets.BREVO_LIST_TOUS }}/" dist/api/config.php
          sed -i "s/admin@REPLACE_ME/${{ secrets.ADMIN_EMAIL }}/" dist/api/config.php
```

Also rename the step to reflect it now injects more than just Stripe:

```yaml
      - name: Inject secrets into config.php
```
...
git commit -m "ci: inject Brevo and webhook secrets in production deploy"
```

---

### Task 4: Update deploy-test.yml for staging secrets

**Files:**
- Modify: `.github/workflows/deploy-test.yml`

**Step 1: Add sed commands after the existing Stripe config injection**

In `.github/workflows/deploy-test.yml`, after the existing "Inject Stripe config for staging" step (line 24-27), add the new sed commands to that same step:

```yaml
      - name: Inject secrets for staging
        run: |
          sed -i "s/sk_test_REPLACE_ME/${{ secrets.STRIPE_SECRET_KEY_TEST }}/" dist/api/config.php
          sed -i "s|\$base_path = '';|\$base_path = '/test';|" dist/api/config.php
          sed -i "s/whsec_REPLACE_ME/${{ secrets.STRIPE_WEBHOOK_SECRET_TEST }}/" dist/api/config.php
          sed -i "s/xkeysib-REPLACE_ME/${{ secrets.BREVO_API_KEY }}/" dist/api/config.php
          sed -i "s/\$brevo_list_adherents = 0/\$brevo_list_adherents = ${{ secrets.BREVO_LIST_ADHERENTS }}/" dist/api/config.php
          sed -i "s/\$brevo_list_donateurs = 0/\$brevo_list_donateurs = ${{ secrets.BREVO_LIST_DONATEURS }}/" dist/api/config.php
          sed -i "s/\$brevo_list_tous = 0/\$brevo_list_tous = ${{ secrets.BREVO_LIST_TOUS }}/" dist/api/config.php
...
git commit -m "ci: inject Brevo and webhook secrets in staging deploy"
```

---

### Task 5: Update CLAUDE.md with new GitHub Actions secrets

**Files:**
- Modify: `CLAUDE.md`

**Step 1: Add new secrets to the Secrets GitHub Actions section**

In `CLAUDE.md`, update the "Secrets GitHub Actions" section to include the new secrets:
...
Note: `STRIPE_SECRET_KEY_LIVE` and `STRIPE_SECRET_KEY_TEST` are already used in the workflows but were not previously documented. Add them for completeness.

**Step 2: Commit**

```bash
git add CLAUDE.md
git commit -m "docs: add webhook and Brevo secrets to CLAUDE.md"
```
...
**Step 1: Run the Astro build to make sure nothing is broken**

```bash
npm run build
```

Expected: Build succeeds. The `dist/api/` directory should contain both `create-checkout.php`, `config.php`, and `stripe-webhook.php`.

**Step 2: Verify the files are in dist**

```bash
ls -la dist/api/
```

Expected: `config.php`, `create-checkout.php`, `stripe-webhook.php` all present.

**Step 3: Verify PHP syntax**

```bash
php -l dist/api/stripe-webhook.php
